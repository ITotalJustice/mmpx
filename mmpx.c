    ...
    for (uint32_t srcY = 0; srcY < srcHeight; ++srcY)
    {
        ...
        for (srcX = 0; srcX < srcWidth; ++srcX)
        {
            ...
            if (((A ^ E) | (B ^ E) | (C ^ E) | (D ^ E) | (F ^ E) | (G ^ E) | (H ^ E) | (I ^ E)) != 0)
            {
                ...
                // Intersection rules
                {
                    if ((E != F && all_eq4(E, C, I, D, Q) && all_eq2(F, B, H)) && (F != src(&meta, srcX + 3, srcY))) K = M = F;
                    if ((E != D && all_eq4(E, A, G, F, R) && all_eq2(D, B, H)) && (D != src(&meta, srcX - 3, srcY))) J = L = D;
                    if ((E != H && all_eq4(E, G, I, B, P) && all_eq2(H, D, F)) && (H != src(&meta, srcX, srcY + 3))) L = M = H;
                    if ((E != B && all_eq4(E, A, C, H, S) && all_eq2(B, D, F)) && (B != src(&meta, srcX, srcY - 3))) J = K = B;
                ...
                // 2:1 slope rules
                {
                    ...
                    if (F != D)
                    {
                        ...
                        if (F != E && F != A && F != G)
                        {
                            if (all_eq3(F, C, H, S) && none_eq2(F, B, src(&meta, srcX - 1, srcY + 2))) K = M;
            ...
            *dstPacked = M;
            ...
            A = B; B = C; C = src(&meta, srcX + 2, srcY - 1);
            Q = D; D = E; E = F; F = R; R = src(&meta, srcX + 3, srcY);
            G = H; H = I; I = src(&meta, srcX + 2, srcY + 1);
        } // X
    } // Y
}
